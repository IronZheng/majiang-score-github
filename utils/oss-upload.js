...
// ---------- MD5（与后端 MessageDigest(UTF-8) 一致的参考实现） ----------
// 来源：blueimp/JavaScript-MD5（RFC 1321），经验证与 Node crypto / 后端一致。
function md5(input) {
  ...
  function rstr2hex(input) {
    ...
    for (var i = 0; i < input.length; i++) {
      var xchar = input.charCodeAt(i);
      output += hexTab.charAt((xchar >>> 4) & 0x0f) + hexTab.charAt(xchar & 0x0f);
    }
    return output;
  }
